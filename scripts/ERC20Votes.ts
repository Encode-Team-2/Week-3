...
async function main() {
  ...
  console.log(
    `Account ${
      acc1.address
    } had ${pastVotes1.toString()} units of voting power at previous block\n`
  );
  console.log(`Current block number is ${blockNumber - 1}\n`);
  const pastVotes2 = await contract.getPastVotes(acc1.address, blockNumber - 2);
  console.log(
    `Account ${
      acc1.address
    } had ${pastVotes2.toString()} units of voting power at previous block\n`
  );
  console.log(`Current block number is ${blockNumber - 2}\n`);
